...
class HTTPCookieProxyAPIDelegate: PigeonApiDelegateHTTPCookie {
  func pigeonDefaultConstructor(
    ...
    let registrar = pigeonApi.pigeonRegistrar as! ProxyAPIRegistrar
      var keyValueTuples: [(HTTPCookiePropertyKey, Any)] = []
      for (key, value) in properties {
        let newKey: HTTPCookiePropertyKey
        switch key {
          case .comment: newKey = .comment
          case .commentUrl: newKey = .commentURL
          case .discard: newKey = .discard
          case .domain: newKey = .domain
          case .expires: newKey = .expires
          case .maximumAge: newKey = .maximumAge
          case .name: newKey = .name
          case .originUrl: newKey = .originURL
          case .path: newKey = .path
          case .port: newKey = .port
          case .secure: newKey = .secure
          case .value: newKey = .value
          case .version: newKey = .version
          case .sameSitePolicy:
            if #available(iOS 13.0, macOS 10.15, *) {
              newKey = .sameSitePolicy
            } else {
              throw registrar.createUnsupportedVersionError(
                method: "HTTPCookiePropertyKey.sameSitePolicy",
                versionRequirements: "iOS 13.0, macOS 10.15")
            }
          case .unknown:
            throw registrar.createUnknownEnumError(withEnum: key)
        }
        keyValueTuples.append((newKey, value)) 
      }
